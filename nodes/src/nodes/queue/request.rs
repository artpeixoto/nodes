...
use either::Either;
use heapless::{FnvIndexMap, FnvIndexSet, IndexMap};
use crate::base::node::{Node, NodeRef};
use crate::base::proc::process_errors::NodeBorrowError;
use crate::common_types::request::QueueRequestQueryError::RequestNotFound;
use crate::common_types::request::Request::{Answer, Processing};
...
pub type RequestNodeRef<'a, TReq, TAns> = NodeRef<'a, Option<Request<TReq, TAns>>>;
...
struct RequestQueueKernel<TReq, TAns, const QUEUE_SIZE: usize>{
...
impl<TReq, TAns, const QUEUE_SIZE: usize> RequestQueueKernel<TReq, TAns, QUEUE_SIZE> {
    fn new() -> Self{
        Self{
            last_identifier: 0,
    ...
    fn get_req(&self, req_id: RequestIdentifier) -> Option<Request<&TReq, &TAns>> {
        ...
        unsafe {
            ...
            match &header.situation {
                ...
                RequestSituation::Processing => Request::Processing,
...
pub struct RequestQueueNode<TReq, TAns, const QUEUE_SIZE: usize> {
    kernel_node: Node<RequestQueueKernel<TReq, TAns, QUEUE_SIZE>>
}

impl<TReq, TAns, const QUEUE_SIZE: usize> RequestQueueNode<TReq, TAns, QUEUE_SIZE> {
    pub fn new() -> Self {
        Self{
            kernel_node: Node::new(
                RequestQueueKernel::new()
            )
        }
    }
...
pub enum QueueRequestQueryError {
    CantOpenKernel(NodeBorrowError),
    RequestNotFound
}
impl From<NodeBorrowError> for QueueRequestQueryError{
    fn from(value: NodeBorrowError) -> Self {
        QueueRequestQueryError::CantOpenKernel(value)
    }
}


impl From<BorrowError> for QueueRequestQueryError{
    fn from(value: BorrowError) -> Self {
        QueueRequestQueryError::CantOpenKernel(NodeBorrowError::from(value))
    }
}

impl From<BorrowMutError> for QueueRequestQueryError{
...
pub struct RequestQueueClientEndpoint<'a, TReq, TAns, const QUEUE_SIZE: usize>{
    kernel_ref: NodeRef<'a, RequestQueueKernel<TReq, TAns, QUEUE_SIZE>>
}
        ...
        (Either<NodeBorrowError, PutAnswerError>, TAns)>
    {
        let mut kernel_ref =
            match  self.kernel_ref.try_borrow_mut(){
                ...
                Err(borrow_err) =>  {return Err((Either::Left( NodeBorrowError::from(borrow_err)
                ), ans));}
            };
